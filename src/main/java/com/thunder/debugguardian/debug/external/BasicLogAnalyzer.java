...
public class BasicLogAnalyzer implements LogAnalyzer {
    ...
    public String analyze(List<ThreadReport> threads) {
        ...
        boolean allUnknown = threads.stream()
                .allMatch(tr -> tr.mod().equalsIgnoreCase("unknown"));

        if (allUnknown) {
            sb.append("\nNo mod-specific stack frames were detected; the process may have been terminated before a crash could be logged.\n");
        } else {
